package org.mathison.multirframework.argumentidentification;
...
public class NERArgumentIdentification implements ArgumentIdentification {
	...
	public List<Argument> identifyArguments(Annotation d, CoreMap s) {
		...
		for(List<CoreLabel> argumentTokenSpan : argumentTokenSpans){
			...
			int tokenStartOffset = argumentTokenSpan.get(0).get(TokenOffsetInformation.SentenceRelativeCharacterOffsetBeginAnnotation.class);
			int tokenEndOffset = argumentTokenSpan.get(argumentTokenSpan.size()-1).get(TokenOffsetInformation.SentenceRelativeCharacterOffsetEndAnnotation.class);
