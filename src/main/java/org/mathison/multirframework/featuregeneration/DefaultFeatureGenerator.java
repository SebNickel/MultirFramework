package org.mathison.multirframework.featuregeneration;
...
import org.mathison.multirframework.corpus.TokenOffsetInformation;
...
public class DefaultFeatureGenerator implements FeatureGenerator {
	...
	public List<String> generateFeatures(Integer arg1StartOffset,
		...
		for(int i =0; i < tokens.size(); i++){
			...
			int begOffset =token.get(TokenOffsetInformation.SentenceRelativeCharacterOffsetBeginAnnotation.class);
			int endOffset = token.get(TokenOffsetInformation.SentenceRelativeCharacterOffsetEndAnnotation.class);
		...
		List<Triple<Integer,String,Integer>> dependencyData = sentence.get(SentDependencyInformation.DependencyAnnotation.class);
